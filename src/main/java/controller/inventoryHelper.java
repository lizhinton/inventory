...
public class inventoryHelper {
	...
	public void deleteItem(inventory toDelete) {
		EntityManager em = emfactory.createEntityManager();
		em.getTransaction().begin();
		TypedQuery<inventory> typedQuery = em.createQuery("select i from inventory i where i.id = :selectedId",
				inventory.class);
	...
	public List<inventory> showAllInventory() {
		EntityManager em = emfactory.createEntityManager();
		List<inventory> allInventory = em.createQuery("SELECT i FROM inventory i").getResultList();
